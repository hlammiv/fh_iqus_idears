...
def build_model(Lx, Ly, J=1.0, U=0.0, periodic=True):
    ...
    for (a, b) in ff.lattice(Lx, Ly, periodic):
        for sp in ("u", "d"):
            cm.add_coupling_term(-J, a, b, f"Cd{sp} JW", f"C{sp}",
...
def build_state(Lx, Ly, site):
    """The dimer covering, one doublon and one holon -- same state as free_fermion.

    from_singlets gives (|up down> - |down up>)/sqrt(2) per pair, which is exactly
    free_fermion's (|A_T> - |B_T>)/sqrt(2). The two lonely sites come out 'empty';
    the doublon is then filled with Cdu Cdd, whose two Jordan-Wigner strings are
    identical and cancel, so no string bookkeeping is needed.
    """
    from tenpy.networks.mps import MPS
    dimers, doublon, holon, spare = ff.covering(Lx, Ly)
    if spare:
...
def czz_exact_free(Lx, Ly, t, links, periodic=True):
    order, idx, dimers, doublon, holon, spare = ff.mode_order(Lx, Ly)
    h = ff.hopping(Lx, Ly, idx, periodic=periodic)
    nu, nu2 = ff.occupations(idx, dimers, doublon, holon, spare)
    return [ff.czz_free(t, h, nu, nu2, idx, dimers, i, j) for (i, j) in links]
...
def validate(sizes, times, max_size=5e7):
    """Exact diagonalisation of the MPO -- no TDVP, so this tests only the setup."""
    from tenpy.algorithms.exact_diag import ExactDiag
    from tenpy.linalg import np_conserved as npc
    rows, worst = [], 0.0
    for (Lx, Ly) in sizes:
        model, site = build_model(Lx, Ly)
        psi, dimers = build_state(Lx, Ly, site)
        ...
        for t in times:
            ...
            ref = czz_exact_free(Lx, Ly, t, dimers)
            err = max(abs(a - b) for a, b in zip(got, ref))
            worst = max(worst, err)
            rows.append({"Lx": Lx, "Ly": Ly, "t": t, "max_abs_err": err,
...
def evolve(Lx, Ly, t_final, dt, chi, U=0.0, svd_min=1e-14):
    from tenpy.algorithms.tdvp import TwoSiteTDVPEngine
    model, site = build_model(Lx, Ly, U=U)
    psi, dimers = build_state(Lx, Ly, site)
...
def scan(Lx, Ly, times, dts, chis, U=0.0):
    rows = []
    for t in times:
        ref = czz_exact_free(Lx, Ly, t, ff.covering(Lx, Ly)[0]) if U == 0 else None
        for chi in chis:
            for dt in dts:
                psi, links, chi_used, secs = evolve(Lx, Ly, t, dt, chi, U)
                got = czz_mps(psi, links)
...
def main():
    ...
    if a.validate:
        ...
        rows, ok = validate([(3, 2), (2, 3)], [0.0, 0.1, 0.5, 1.0])
        out["validate"] = rows
        if not ok:
            sys.exit(1)
    if a.scan or a.chi_scan:
        ...
        print(f"\n{a.lx}x{a.ly} doubly periodic, U = {a.u}, "
              f"chi in {chis}, dt in {dts}:")
        rows = scan(a.lx, a.ly, a.times, dts, chis, a.u)
        out["scan"] = rows
        report(rows)
    if a.json:
        p = DATA / a.json
