...
- Model `2c980fb57752`. Every number below is read out of `RESULTS.json`; regenerate with `python3 make_record.py`.
...
- At n = 10^6: NISQ+PEC 16, STAR 27, surface FT 26, Pinnacle 39.
- Clearing the ED frontier: Pinnacle 5.56e+05, STAR 8.58e+05, NISQ+PEC not reached below n = 1e+10, surface FT 1.04e+06.
...
- Converged answers: at t_max = sqrt(m)/v the Lieb-Robinson buffer is self-defeating -- the best arm at n = 10^6 reaches m = 39, whose own t_max = 3.11 would need m >= 2181 to CERTIFY a thermodynamic-limit answer. Nothing on the figure closes that loop, classical included. A bound failing is not proof of non-convergence (`OPEN_ITEMS.md` O8).
